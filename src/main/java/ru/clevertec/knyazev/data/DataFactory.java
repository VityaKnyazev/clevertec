...
public class DataFactory {
	...
	private DataReader getDataReader() throws IOException {
		InputSource inputSource = InputSource.valueOf(args[0].toUpperCase());

		DataReader dataReader = switch (inputSource) {
		case CONSOLE -> new ConsoleDataReader();
		case DATABASE -> new DatabaseDataReader();
		case STANDARD -> {
			if (args.length < 3) {
				throw new IOException(
	...
	private DataWriter getDataWriter() throws IOException {
		...
		OutputSource outputSource = OutputSource.valueOf(args[1].toUpperCase());
		
		DataWriter dataWriter = switch(outputSource) {
		case  CONSOLE -> new ConsoleDataWriter();
		case FILE -> new FileDataWriter(outputFileName);
		default -> throw new IOException("Invalid input source value: " + args[1]);
		};
		return dataWriter;
	}
	
	private static enum InputSource {
		STANDARD, CONSOLE, FILE, DATABASE
	}

	private static enum OutputSource {
		CONSOLE, FILE
	}	
